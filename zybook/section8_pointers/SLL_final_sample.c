...
int main(){
    ...
    while (choice != 8){
        switch (choice){
            case 1:
                printf("%s", "Enter a name (Max 14 characters): ");
                fgets(item, 15, stdin);
                ...
                setData(newPtr, item);
                insert(newPtr); // insert item in the list
                printList();
                break;
            
            case 2:
                if(!isEmpty()){
                    printf("%s", "Enter name to be deleted: ");
                    fgets(item, 15, stdin);
...
void insert(Node* newPtr) {
    ...
    // Check for inserting at the start or if the list is empty
    if (start == NULL || strcmp(start->data, "#") == 0 || strcasecmp(newPtr->data, start->data) <= 0) {
        newPtr->next = start;
        start = newPtr;
    } else {
        current = start;
        ...
        while (current != NULL && strcasecmp(newPtr->data, current->data) > 0) {
            previous = current;
            current = current->next;
        }
